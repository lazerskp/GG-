...
export async function generateMetadata({ params }: ArtistPageProps): Promise<Metadata> {
  ...
  const title = `${data.artist.name} | GULLYGANG`;
  const canonicalUrl = `${SITE_URL}/artist/${encodeURIComponent(id)}`;
...
export default async function ArtistPage({ params }: ArtistPageProps) {
  ...
  const relatedArticles = getArticlesByArtist(artist.id, artist.name);
